...
function ProfilePage() {
	...
	return (
		<div className="p-8 max-w-2xl mx-auto space-y-8">
			<div className="border rounded-md border-primary p-4 space-y-1">
			...
			<div className="border rounded-md border-primary p-4 space-y-4">
				<h2 className="text-xl font-medium">My Bidets</h2>
				<QueryResolver query={locationsQuery}>
					{(locations) =>
						locations.length === 0 ? (
							<p className="text-muted-foreground">You haven't added any bidets yet.</p>
						) : (
							<ul className="space-y-3">
								{locations.map((location) => (
									<li key={location.id} className="flex items-center gap-2 text-lg">
										...
										<span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusStyles[location.status]}`}>
